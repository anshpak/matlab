function res = twin_2(h)
% Задание 2. Разработайте программу (m-сценарий), позволяющую пользователю 
% задавать с помощью мыши два отрезка A и B, после чего выполните твининг 
% отрезка A в отрезок B с отображением исходного отрезка A, целевого отрезка 
% B и текущего промежуточного изображения (твина) между A и B при изменении 
% параметра t от 0 до 1 с заданным шагом.
clf
axis([-1 1 -1 1])
